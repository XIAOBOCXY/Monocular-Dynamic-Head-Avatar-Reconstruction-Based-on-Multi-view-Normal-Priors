...
class DINOBase(nn.Module):
    ...
    def _load_dino_model():
        ...
        if os.path.isdir(local_repo):
            return torch.hub.load(local_repo, 'dinov2_vitb14', pretrained=True, source='local')
        try:
            return torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14', pretrained=True)
        except Exception as exc:
            raise RuntimeError(
                'Failed to load DINOv2. If the server cannot access GitHub, pre-populate the torch hub cache '
                'or keep /root/.cache/torch/hub/facebookresearch_dinov2_main available.'
            ) from exc
