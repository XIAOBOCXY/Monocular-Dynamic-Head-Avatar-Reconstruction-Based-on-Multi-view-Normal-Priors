...
class Trainer:
    def __init__(
        ...
        self._tb_dir = os.path.join(self._dump_dir, 'tensorboard')
        if not debug:
    ...
    def _logger(self, iter_idx, fit_bar, loss_metrics, show_metric):
        ...
        if iter_idx % self._log_interval == 0:
            log_metric_value = self._dict_mean_value(self.log_stats)
            show_metric_value = self._dict_mean_value(self.show_stats)
            ...
            self.writer.add_scalar('train/lr', learning_rate, iter_idx)
            self.writer.add_scalar('train/loss', log_loss, iter_idx)
            for key, value in log_metric_value.items():
                self.writer.add_scalar(f'train/{key}', value, iter_idx)
            for key, value in show_metric_value.items():
                self.writer.add_scalar(f'train_show/{key}', value, iter_idx)
            self.writer.flush()
    ...
    @staticmethod
    def _dict_mean_value(dict_list):
        mean_dict = {}
        for key in dict_list[0].keys():
            mean_dict[key] = float(np.mean([d[key] for d in dict_list]))
        return mean_dict
