...
def inference(image_path, driver_path, resume_path, force_retrack=False, device='cuda', with_watermark=False):
    ...
    if with_watermark:
        ...
        _water_mark = torchvision.io.read_image(
            'demos/gagavatar_logo.png', mode=torchvision.io.ImageReadMode.RGB_ALPHA
        ).float() / 255.0
        _water_mark = torchvision.transforms.functional.resize(
            _water_mark, _water_mark_size, antialias=True
        ).to(device)
    images = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--with_watermark', action='store_true')
